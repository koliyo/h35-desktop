...
from h35_ops.pr_checkout import checkout_pr, list_open_prs, local_pr_branch, main, parse_pr_ref
...
def test_parse_github_pr_url() -> None:
    url = "https://github.com/koliyo/h35-desktop/pull/39"
...
def test_checkout_switches_prefixed_branch(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    monkeypatch.setattr("h35_ops.pr_checkout._git", git)
    monkeypatch.setattr(
        "h35_ops.pr_checkout._gh_head_ref",
...
def test_checkout_refuses_dirty_worktree(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("h35_ops.pr_checkout._git", git)
    monkeypatch.setattr("h35_ops.pr_checkout._gh_head_ref", lambda *_: "feat/example")
...
def test_list_open_prs_runs_gh(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("h35_ops.pr_checkout.subprocess.run", run)
...
def test_main_without_ref_lists_prs(monkeypatch) -> None:
    monkeypatch.setattr("h35_ops.pr_checkout.list_open_prs", lambda: 0)
    monkeypatch.setattr(
        "h35_ops.pr_checkout.checkout_pr",
