from h35_ops.ghutil import DEFAULT_CHECKS
from h35_ops.promote import (
    PROMOTE_TAG_USAGE,
    PROMOTE_USAGE,
    promote_command,
    promote_tag,
...
def test_promote_tag_annotates_and_pushes_origin_main(monkeypatch, tmp_path) -> None:
    calls: list[list[str]] = []
    waited: list[str] = []
    monkeypatch.setattr("h35_ops.promote.repo_root", lambda: tmp_path)
    monkeypatch.setattr(
        "h35_ops.promote.subprocess.run",
    ...
    monkeypatch.setattr("h35_ops.promote.wait_for_promote_ci", waited.append)
...
def test_promote_tag_force_moves_dev(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("h35_ops.promote.wait_for_promote_ci", lambda sha: None)
...
def test_promote_tag_does_not_push_when_ci_fails(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        "h35_ops.promote.wait_for_promote_ci",
...
def test_wait_for_promote_ci_waits_default_checks(monkeypatch) -> None:
    seen: list[str] = []
    monkeypatch.setattr("h35_ops.promote.github_repo", lambda: "koliyo/h35-desktop")
    monkeypatch.setattr("h35_ops.promote.gh_run", lambda args: type("R", (), {"stdout": ""})())
    monkeypatch.setattr(
        "h35_ops.promote.wait_for_check",
